...
from .utils import load_state_dict, count_params, pl_on_train_start, get_obj_from_str, instantiate_from_config
from src.module.loss.vgg_loss import Vgg19, build_vgg_loss
from src.module.ppocr.ppocr_wrapper import PPOCRv4Model
...
class BaseTrainer(pl.LightningModule):
    def __init__(self, baseconfig):
        ...
        self.noise_scheduler = get_obj_from_str(self.config.noise_scheduler).from_config(self.config.scheduler_config)
        ...
        print("Initialize PP-OCRv4 model from pretrained…")

        self.ocr_model = PPOCRv4Model(self.config.ocr_model)
    ...
    def shared_step(self, batch, batch_idx, stage="train"):
        ...
        if loss_names == 'lm':
            ...
            loss, mse_loss, reconstruction_loss = loss
            self.log(
                f"{stage}_traj/loss",
                loss,
                batch_size=len(batch["img"]),
    ...
    def prepare_input(self, batch):
        ...
        return {"noise": noise, "timestep": t, "latent": zt, "cond": c}
    ...
    def forward(self, batch):
        ...
        if self.config.reconstruction_loss:
            ...
            loss = mse_loss + reconstruction_loss
            return (loss, mse_loss, reconstruction_loss), 'lmr'
        else:
            loss = mse_loss
            return (loss, mse_loss), 'lm'
